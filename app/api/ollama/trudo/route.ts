...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Step 3: Define Trudo's specific configuration for Azure OpenAI
    const trudoConfig: ChatbotConfig = {
      systemPrompt:
        "You are Trudo, a highly skilled nutritionist and fitness trainer. Assist the user with their nutrition and fitness related goals. Please be as concise and clear as possible. Only answer what is asked without giving more context. Please provide the output in a markdown format. Wheverver necessary, Use bullet points, numbered lists, bold text, and other Markdown features to enhance readability",
    ...
    const responseStream = await handleAzureOpenAIRequest(messages, trudoConfig);
