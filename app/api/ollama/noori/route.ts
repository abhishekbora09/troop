...
export async function POST(request: NextRequest) {
  ...
  console.log("Noori API route POST request received.");
  ...
  try {
    ...
    // Step 3: Define Noori's specific configuration for Azure OpenAI
    const nooriConfig: ChatbotConfig = {
      systemPrompt:
        "You are Noori, a highly skilled nutritionist and fitness trainer. Assist the user with their nutrition and fitness related goals. Please be as concise and clear as possible. Only answer what is asked without giving more context. Please provide the output in a markdown format. Wheverver necessary, Use bullet points, numbered lists, bold text, and other Markdown features to enhance readability",
    ...
    const responseStream = await handleAzureOpenAIRequest(messages, nooriConfig);
    console.log("Received response stream from handleAzureOpenAIRequest.");

    // Step 5: Log the total processing time
    const totalEndTime = Date.now();
    console.log(`Noori API route processing time: ${totalEndTime - totalStartTime} ms.`);
    ...
    console.log(`Noori API route failed after ${errorTime - totalStartTime} ms.`);
    console.error("Error in Noori API route:", error);
